...
public class LeafParts {
	...
	public static void getLeafPartCounts(String upperPart, Statement stmt, Map<String, Integer> leafPartCounts,
			int prod) throws SQLException {
		...
		try (PreparedStatement pstmt = stmt.getConnection().prepareStatement(sql)) {
			...
			while (rs.next()) {
				...
				getLeafPartCounts(lowerPart, stmt, leafPartCounts, prod * quantity); // Calling the function recursively
																						// for lower part with
																						// corresponsing quantity
			}
			if (isPartLeaf) {
